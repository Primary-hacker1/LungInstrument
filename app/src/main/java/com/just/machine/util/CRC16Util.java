...
public class CRC16Util {
    ...
    public static String getCrc(int res) {
        ...
        String substring = format.substring(0, 2).toUpperCase();
        String substring1 = format.substring(2, 4).toUpperCase();
        //Log.i("BLUEDATA", "crc ---- : " + substring + "  " + substring1);
        return substring1.concat(" ").concat(substring).concat(" ");
    }

    public static String getCrcTo(int res) {
        String format = String.format("%04x", res);
        String substring = format.substring(0, 2).toUpperCase();
        String substring1 = format.substring(2, 4).toUpperCase();
        //Log.i("BLUEDATA", "crc ---- : " + substring + "  " + substring1);
     ...
     * @return
     */
    public static boolean checkCrcGet(byte[] bytes) {
        int a = calcCrc16(bytes, 0, bytes.length - 2);
        String str1 = getCrc(a);
        String s11 = str1.substring(0, 2);
        String s22 = str1.substring(3, 5);
        String s = bytesToHexString(bytes);
        String s111 = s.substring(s.length() - 4, s.length() - 2);
        String s222 = s.substring(s.length() - 2, s.length());
        //System.out.println(s11+"+"+s22);
        if (s11.equals(s111) && s22.equals(s222)) {
            return true;
        }
        return false;
    }
    ...
    public static byte[] getByte(byte[] bytes) {
        ...
        String s11 = str.substring(6, str.length());
        byte[] bytes1 = hexStringToBytes(s11);
        return bytes1;
    }
     ...
     * @return
     */
    public static byte[] checkCrcTo(byte[] bytes, String head) {
        String byteStr = CRC16Util.bytesToHexString(bytes);
        int lenInt = bytes.length;
        if (lenInt < 0) {
            lenInt = lenInt + 256;
        }
        int d1 = lenInt / 16;
        int d2 = lenInt % 16;
        byteStr = head + HEX_CHAR[d1] + HEX_CHAR[d2] + byteStr;
        byte[] crcByte = CRC16Util.hexStringToBytes(byteStr);
        int crcInt = CRC16Util.calcCrc16(crcByte);
        String crcStr = CRC16Util.getCrcTo(crcInt);
        String strStr = byteStr + crcStr;
        byte[] crcbyte = CRC16Util.hexStringToBytes(strStr);
        return crcbyte;
    }
